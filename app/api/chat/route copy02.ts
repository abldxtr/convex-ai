...
export async function POST(req: Request) {
  ...
  if (!getChat?.chatItem) {
    ...
    await fetchMutation(
      ...
      { id: streamId, chatId, userId: userId._id },
      { token }
    );
    ...
    await fetchMutation(
      api.stream.createStream,
      { id: streamId, chatId: body.chatId, userId: userId._id },
      { token }
    );
    const stream = createDataStream({
      execute: (dataStream) => {
...
export async function GET(request: Request) {
  ...
  let streamIds: Doc<"stream"> | null;

  try {
    let chatid = chatId as Id<"chats">;
  ...
  const recentStreamId = streamIds._id;
